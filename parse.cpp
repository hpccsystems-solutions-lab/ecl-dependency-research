...
struct node {
    ...
    // right - varibles on the right side on IR expression (or) the parameters of the operation involved
    // func - the operation performed in the given IR staement
    // cnst - constant type token (or) field name string corresponding to the declaration type token
    // annotation : label string; present inside {} in a annotation type token
};

vector < pair < string, struct node >> mp; // stores all lines of IR in struct node format
unordered_map < string, pair < string, string >> fieldMapper; // mapping IR token to values(which can be fields, variables, constants, etc) in souce code
unordered_map < string, vector < string >> transforms; // translation of processed parameters used in transform, transform list
unordered_map < string, int > counts; // allows for different names for multiple instances same operations  
unordered_map < string, string > records; // mapping IR token to the list of fields
unordered_map < string, string > recName_to_structure; // mapping dataset name to its strucutre
...
set < pair < string, string >> e; //collection of edges in the graph
unordered_map < string, string > compound_nodes; // association of related nodes 
vector < vector < string >> labels; //edge labels
unordered_map < string, string > mapper_to_name; // mapping between operations and nodes associated with the operation
unordered_map < string, string > name_to_mapper;

/* 
addedge : used to create the graph from the processing done before, recursively
*/
void addedge(string node, string present, unordered_map < string, struct node > & mp2, string & compound_parent) {
    ...
    if (mp2[node].annotation.substr(0, 6) == "symbol") {
        ...
        if (str == "newusertable") {
            ...
            for (auto p: mp2[node].right)
                addedge(p, present, mp2, compound_parent);
            return;
        } else if (str == "transformlist") {
            for (auto p: mp2[node].right)
                addedge(p, present, mp2, compound_parent);
            return;
        } else if (str == "sort") {
            ...
            if (str == "filter") {
                ...
                mapper_to_name[str] = fieldMapper[node].first;
                name_to_mapper[fieldMapper[node].first] = str;
                labels.push_back(vector < string > {
                    str,
                    fieldMapper[mp2[node].right[1]].first,
                    fieldMapper[mp2[node].right[1]].second
                });
                e.insert({
                    fieldMapper[mp2[node].right[1]].first,
                    str
            ...
            } else str += to_string(++counts["c_count"]), mapper_to_name[str] = fieldMapper[node].first, name_to_mapper[fieldMapper[node].first] = str;
            addedge(mp2[node].right[0], str, mp2, compound_parent);
        } else if (mp2[node].func[0] == "hqlproject") { // OPERATION is "hqlproject" in IR statement with index "node"

            str = "Project(DATASET : " + fieldMapper[mp2[node].right[0]].first + " , ";
            str += "TRANSFORM : " + fieldMapper[mp2[node].right[1]].first + ")";

            // <source node label,destination node label,edge label> tuple for an edge
            labels.push_back(vector < string > {
                present,
                str,
                fieldMapper[node].second
            ...
            e.insert({
                str,
                present
            }); // source destination node labels as a pair

            for (auto x: mp2[node].right)
                addedge(x, str, mp2, compound_parent); // recursively traverse the "right" tokens
        } else if (str == "normalize" || str == "denormalize" || str == "output" || str == "join" || str == "rollup") {
            string tmp, s = str;
            if (str == "normalize") {
                str += to_string(++counts["norm_count"]);
                mapper_to_name[str] = fieldMapper[node].first;
                name_to_mapper[fieldMapper[node].first] = str;
            }
            if (str == "denormalize") {
                str += to_string(++counts["denorm_count"]);
                mapper_to_name[str] = fieldMapper[node].first;
                name_to_mapper[fieldMapper[node].first] = str;
            }
            if (str == "rollup") str += to_string(++counts["rollup_count"]), mapper_to_name[str] = fieldMapper[node].first, name_to_mapper[fieldMapper[node].first] = str;
        ...
        if (str == "eq" || str == "if" || fieldMapper[node].second == "logical_operation") {
            ...
            e.insert({
                fieldMapper[node].first,
                present
            });
        } else if (str != present && str != "transform") {
            if (present.substr(0, 4) != "norm") labels.push_back(vector < string > {
                present,
                str,
                fieldMapper[node].second
            });
            else labels.push_back(vector < string > {
                present,
                str,
                "ITERATIONS"
            });
...
int main(int argc, char ** argv) {
    ...
    string inp_file = "ir.txt";
    string op_file = "temp.js";

    ofstream File(op_file);
    ...
    for (string c: txt) {
        ...
        RIGHT : %c2 %e21 %e23 
        OPERATION : table 
        OUTPUTS(cumulative including this statement) : %e173 %e110 
        ANNOTATION : 
        CONSTANT : 
        */
        while (i < (int) c.size()) {
    ...
    //code to parse and exctract info out of the nodes created, string and character parsing
    ...
    for (int i = (int) mp.size() - 1; i >= 0; i--) {
        ...
        if (y.annotation.substr(0, 6) == "symbol") {
            ...
            if (fieldMapper[y.right[0]].second == "inlinetable") {
                string
                var = fieldMapper[y.right[0]].first;
        ...
        else if (y.right.size() > 0 && y.right[0].substr(0, 7) == "counter") fieldMapper[le] = {
            "AUTO_INCREMENT",
            "counter"
        };
            ...
            //
                ...
                string recField;
                if (fieldMapper[y.right[0]].first != "SELF")
                    recField = fieldMapper[y.right[0]].first + "." + fieldMapper[y.right[1]].first;
                else
                    recField = fieldMapper[y.right[1]].first;
                fieldMapper[le] = make_pair(recField, "select");
            } else if ((func == "left" || func == "right" || func == "implicitcast")) {
                fieldMapper[le] = fieldMapper[y.right[0]];
            } else if (func == "substring") {
                fieldMapper[le] = {
                    "substring(" + fieldMapper[y.right[0]].first + ")",
                    "substring"
                };
            } else if (func == "count") {
                fieldMapper[le] = {
                ...
                fieldMapper[le] = {
                    "IF(" + fieldMapper[y.right[0]].first + ")",
                    "conditional"
                };
            } else if (func == "and" || func == "or") {
                fieldMapper[le] = {
                    fieldMapper[y.right[0]].first + " " + func + " " + fieldMapper[y.right[1]].first,
                    "logical_operation"
                };
            ...
            // -----------
                ...
                fieldMapper[le] = {
                    fieldMapper[y.right[0]].first + op + fieldMapper[y.right[1]].first,
                    "conditional"
                };
            } else if (func == "filter") {
                fieldMapper[le] = {
                    "RECORDSET({x where " + fieldMapper[y.right[1]].first + "})",
                    "filter"
                };
            } else if (func == "normalize") {
                string op = "Nx`x``ORMALIZED(\n";
                for (auto z: y.right) {
    ...
    for (int i = (int) mp.size() - 1; i >= 0; i--) {
        ...
        if (y.func.size() > 0) {
            string s;
            if (fieldMapper[x.first].second == "count" || fieldMapper[x.first].second == "filter" || fieldMapper[x.first].second == "normalize") s = "`" + name_to_mapper[fieldMapper[x.first].first] + "`";
            else s = "`" + fieldMapper[x.first].first + "`";
            if (y.func[0] == "select") {
    ...
    for (auto x: e) {
        ...
        if (x.first.size() == 0 || x.second.size() == 0) continue;
